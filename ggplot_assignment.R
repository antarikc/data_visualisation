...
sepal_plot1 <- ggplot(iris, aes(x=iris$Sepal.Length , y=iris$Sepal.Width , col=iris$Species))+geom_point()
...
sepal_plot2<-ggplot(iris, aes(x=iris$Sepal.Length , y=iris$Sepal.Width , col=iris$Species , size=iris$Petal.Length))+geom_point()
sepal_plot2
...
ggplot(mtcars, aes(x=wt , y=mpg , label=cyl))+geom_point(col="blue")+geom_text(aes(label=cyl))
...
ggplot(mtcars , aes(x=mpg , fill=factor(cyl)))+geom_histogram(binwidth = 2)
